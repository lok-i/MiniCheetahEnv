...
class MiniCheetahEnv(gym.Env):
	...
	def __init__(self,render = True,on_rack = False,stairs = False):
		...
		self.robot = pybullet.loadURDF(model_path, self.INIT_POSITION,self.INIT_ORIENTATION)
        ...
        '''
		pybullet.resetBasePositionAndOrientation(self.robot, self.INIT_POSITION, self.INIT_ORIENTATION)
		pybullet.resetBaseVelocity(self.robot, [0, 0, 0], [0, 0, 0])
		pybullet.resetDebugVisualizerCamera(self._cam_dist, self._cam_yaw, self._cam_pitch, [0, 0, 0])

		if self._on_rack:
			pybullet.createConstraint(
				self.robot, -1, -1, -1, pybullet.JOINT_FIXED,
				[0, 0, 0], [0, 0, 0], [0, 0, 0.3])
		elif(self._is_stairs):
			boxHalfLength = 0.1
			boxHalfWidth = 1
			boxHalfHeight = 0.015
	...
	def vis_foot_traj(self,line_thickness = 5,life_time = 15):
		...
		for  link_id in LINK_ID:
			if(link_id!=0):
				current_point = self._pybullet_client.getLinkState(self.stoch2,link_id)[0]
	...
	def reset(self):
		...
		for  link_id in LINK_ID:
			if(link_id!=0):
				self.prev_feet_points[i] = np.array(self._pybullet_client.getLinkState(self.stoch2,link_id)[0])
			else:
	...
	def transform_action(self, action):
		...
		action[16:20] = action[16:20]*0.04				# Max allowed Z-shift due to abduction limits is 3.5cm
		action[17] = -action[17]
		action[19] = -action[19]
		return action
